      ...
      <div className="grid lg:grid-cols-4 gap-8 max-w-7xl mx-auto">
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="space-y-3">
            <h3 className="text-lg font-bold text-slate-300 mb-2 border-b border-slate-800 pb-2">{copy.activePlans}</h3>
            {loading ? (
            ...
            plans.map(plan => (
              <button 
                ...
                className={`w-full ${lang === 'ar' ? 'text-right' : 'text-left'} p-4 rounded-xl border transition-all ${activePlan === plan.id ? 'bg-orange-500/10 border-orange-500/50 text-orange-300 shadow-lg' : 'bg-slate-900/50 border-slate-800 text-slate-400 hover:bg-slate-800'}`}
              >
        ...
        <div className="lg:col-span-3 space-y-6">
           <div className="bg-slate-900/60 p-8 rounded-3xl border border-slate-800 shadow-xl">
              <div className="flex justify-between items-center mb-6 border-b border-slate-800 pb-4">
                 <h3 className="text-2xl font-bold text-slate-100 flex items-center gap-2"><FileSignature className="w-6 h-6 text-orange-400" /> {copy.planDetails}</h3>
                 {active && (
           ...
           <div className="grid md:grid-cols-2 gap-6">
               <div className="bg-orange-900/10 p-6 rounded-3xl border border-orange-500/20 flex flex-col items-center justify-center text-center shadow-[0_0_30px_rgba(249,115,22,0.05)] relative overflow-hidden group">
                  <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center z-10">
                  ...
                  <h3 className="text-lg font-bold text-orange-300">{copy.chartTitle}</h3>
                  <p className="text-xs text-orange-200/60 mt-2 font-mono">
                    {active ? copy.planMeta(active.title, active.intensity) : copy.abcFallback}
                  </p>
               </div>

               <div className="bg-slate-900/60 p-6 rounded-3xl border border-slate-800 flex flex-col justify-center">
                  <h3 className="text-lg font-bold text-slate-200 mb-4 flex items-center gap-2"><BrainCircuit className="w-5 h-5 text-cyan-400" /> {copy.progressTitle}</h3>
                  <p className="text-sm text-slate-400 leading-relaxed p-4 bg-slate-950 rounded-xl border border-slate-800">
                     {copy.progressBody}
                  </p>
               </div>
           </div>
